...
Term.prototype.run = function(connection, options, callback) {
  ...
  if (helper.isPlainObject(connection) && (typeof connection._isConnection === 'function') && (connection._isConnection() === true)) {
    ...
    if (connection._isOpen() === false) {
      return new Promise(function(resolve, reject) {
        reject(new errors.ReqlDriverError('`run` was called with a closed connection', self._query));
      });
    }

    promise = self._run(connection, options);
  } else {
    var poolMaster = self._r.getPoolMaster(); // if self._r is defined, so is self._r.getPool()
    if (!poolMaster) {
      throw new errors.ReqlDriverError('`run` was called without a connection and no pool has been created', self._query);
    }
    ...
    promise = poolMaster.getConnection()
      .then(function(connection) {
        return self._run(connection, options);
      });
  }
        ...
        if ((key === 'readMode') || (key === 'durability') || (key === 'db') ||
          ...
          if (key === 'db') {
            ...
            _options[key] = new Term(self._r).expr(options[key])._query;
                   ...
                   (key !== 'highWaterMark')) {
          reject(new errors.ReqlDriverError('Unrecognized option `' + key + '` in `run`. Available options are readMode <string>, durability <string>, noreply <bool>, timeFormat <string>, groupFormat: <string>, profile <bool>, binaryFormat <bool>, cursor <bool>, stream <bool>'));
          keepGoing = false;
        }
      }
    });
...
Term.prototype.toStream = function(connection, options) {
  ...
  if (!!options.writable)
    return this._toWritableStream.apply(this, args);
  else if (!!options.transform)
    return this._toTransformStream.apply(this, args);
  return this._toReadableStream.apply(this, args);
};

Term.prototype._toReadableStream = function(connection, options) {
  var args;
  var _options = { cursor: true };
  if (helper.isPlainObject(connection) &&
      (typeof connection._isConnection === 'function') && (connection._isConnection() === true)) {
    helper.loopKeys(options, function(obj, key) { _options[key] = obj[key]; });
    args = [connection, _options];
  } else {
    helper.loopKeys(connection, function(obj, key) { _options[key] = obj[key]; });
    args = [_options];
  }

  var stream = new ReadableStream(_options);
  this.run.apply(this, args)
    .then(function(cursor) { stream._setCursor(cursor); })
    .error(function(error) { stream.emit('error', error); });
  return stream;
};
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'primaryKey') &&
          ...
          (key !== 'primaryReplicaTag')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `tableCreate`', self._query, 'Available options are primaryKey <string>, durability <string>, shards <number>, replicas <number/object>, primaryReplicaTag <object>');
      }
    });
...
Term.prototype.indexCreate = function(name, fn, options) {
  ...
  if ((options === undefined) && (helper.isPlainObject(fn))) {
    options = fn;
    fn = undefined;
  }
  ...
  if (helper.isPlainObject(options)) {
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'multi') && (key !== 'geo')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `indexCreate`', self._query, 'Available option is multi <bool> and geo <bool>');
      }
    });
...
Term.prototype.indexRename = function(oldName, newName, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'overwrite') {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `indexRename`', self._query, 'Available options are overwrite <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'squash') && (key !== 'includeStates')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `changes`', self._query, 'Available options are squash <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability') && (key !== 'conflict')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `insert`', self._query, 'Available options are returnChanges <bool>, durability <string>, conflict <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability') && (key !== 'nonAtomic')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `update`', self._query, 'Available options are returnChanges <bool>, durability <string>, nonAtomic <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
...
Term.prototype.replace = function(newValue, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability') && (key !== 'nonAtomic')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `replace`', self._query, 'Available options are returnChanges <bool>, durability <string>, nonAtomic <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `delete`', self._query, 'Available options are returnChanges <bool>, durability <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
...
Term.prototype.table = function(table, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'readMode') {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `table`', self._query, 'Available option is readMode <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'index') && (key !== 'leftBound') && (key !== 'rightBound')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `between`', self._query, 'Available options are index <string>, leftBound <string>, rightBound <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
...
Term.prototype.filter = function(filter, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'default') {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `filter`', self._query, 'Available option is filter');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
...
Term.prototype.eqJoin = function(rightKey, sequence, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'index') {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `eqJoin`', self._query, 'Available option is index <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((keepGoing === true) && (key !== 'index')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `distinct`', self._query, 'Available option is index: <string>');
      }
    });
    if (keepGoing === true) {
      term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  ...
  if (_args.length > 0) {
    if (helper.isPlainObject(_args[_args.length - 1])) {
      helper.loopKeys(_args[_args.length - 1], function(obj, key) {
        if ((key !== 'index') &&
                (key !== 'multi')) {
                    throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `group`', self._query, 'Available options are index: <string>, multi <boolean>');
        }
      });
      term._fillArgs(args);
...
Term.prototype.random = function() {
  ...
  if (_args.length > 0) {
    if (helper.isPlainObject(_args[_args.length - 1])) {
      helper.loopKeys(_args[_args.length - 1], function(obj, key) {
        if (key !== 'float') {
                    throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `random`', self._query, 'Available option is float: <boolean>');
        }
      });
      term._fillArgs(args);
      term._query.push(new Term(self._r).expr(translateOptions(_args[_args.length - 1]))._query);
...
Term.prototype.time = function() {
  ...
  if (foundArgs === false) {
    if ((_args.length !== 4) && (_args.length !== 7)) {
      throw new errors.ReqlDriverError('`r.time` called with ' + _args.length + ' argument' + ((_args.length > 1) ? 's' : ''), null, '`r.time` takes 4 or 7 arguments');
    }
  }
  ...
  var args = [];
  for (var z = 0; z < _args.length; z++) {
    args.push(new Term(this._r).expr(_args[z]));
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'defaultTimezone') {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `ISO8601`. Available options are primaryKey <string>, durability <string>, datancenter <string>');
      }
    });
    term._query.push(new Term(this._r).expr(translateOptions(options))._query);
  }
  ...
  if (expression instanceof Term) {
    ...
    helper.loopKeys(expression, function(expression, key) {
      if (expression[key] !== undefined) {
        ...
        if (optArg instanceof Term && !foundError && !!optArg._error) {
          foundError = true;
    ...
    if (expression === null) {
      ...
      //TODO
      self._error = new errors.ReqlDriverError('Cannot convert `' + expression + '` to datum.');
    }
  }
  return self;
};
...
Term.prototype.json = function(json) {
  ...
  /*
    if ((/\\u0000/.test(json)) || (/\0/.test(json))) {
        this._error = new errors.ReqlDriverError('The null character is currently not supported by RethinkDB');
    }
    */
  var term = new Term(this._r);
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'timeout') &&
          ...
          (key !== 'pageLimit') &&
          (key !== '')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `http`. Available options are reattemps <number>, redirects <number>, verify <boolean>, resultFormat: <string>, method: <string>, auth: <object>, params: <object>, header: <string>, data: <string>, page: <string/function>, pageLimit: <number>');
      }
    });
  ...
  if (helper.isPlainObject(options)) {
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'numVertices') && (key !== 'geoSystem') && (key !== 'unit') && (key !== 'fill')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `circle`', self._query, 'Available options are numVertices <number>, geoSsystem <string>, unit <string> and fill <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'geoSystem') && (key !== 'unit')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `distance`', self._query, 'Available options are geoSystem <string>, unit <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
...
Term.prototype.getIntersecting = function(geometry, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'index') {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `distance`', self._query, 'Available options are index <string>');
      }
    });
    term._query.push(new Term(this._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'index') && (key !== 'maxResults') && (key !== 'maxDist') && (key !== 'unit') && (key !== 'geoSystem')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `getNearest`', self._query, 'Available options are index <string>, maxResults <number>, maxDist <number>, unit <string>, geoSystem <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'waitFor') && (key !== 'timeout')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `wait`', self._query, 'Available options are waitFor: <string>, timeout: <number>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(config)) {
    helper.loopKeys(config, function(obj, key) {
      if ((key !== 'shards') && (key !== 'replicas') &&
          ...
          (key !== 'emergencyRepair')) {
                throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `reconfigure`', self._query, 'Available options are shards: <number>, replicas: <number>, primaryReplicaTag: <object>, dryRun <boolean>, emergencyRepair: <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(config))._query);
  }
  else {
    throw new errors.ReqlDriverError('First argument of `reconfigure` must be an object');
  }
  return term;
};
...
Term.prototype.toString = function() {
  return errors.generateBacktrace(this._query, 0, null, [], { indent: 0, extra: 0 }).str;
};
...
Term.prototype._wrap = function() {
  ...
  if (helper.hasImplicit(this._query)) {
    if (this._query[0] === termTypes.ARGS) {
      throw new errors.ReqlDriverError('Implicit variable `r.row` cannot be used inside `r.args`');
    }
...
Term.prototype._noPrefix = function(term, method) {
  if ((!Array.isArray(term._query)) || (term._query.length > 0)) {
    throw new errors.ReqlDriverError('`' + method + '` is not defined', term._query);
  }
};
...
Term.prototype._arityRange = function(args, min, max, method, term) {
  ...
  if (args.length < min) {
    ...
    if (foundArgs === false) {
      throw new errors.ReqlDriverError('`' + method + '` takes at least ' + min + ' argument' + ((min > 1) ? 's' : '') + ', ' + args.length + ' provided', term._query);
    }
    ...
    if (foundArgs === false) {
      throw new errors.ReqlDriverError('`' + method + '` takes at most ' + max + ' argument' + ((max > 1) ? 's' : '') + ', ' + args.length + ' provided', term._query);
    }
  }
};
...
Term.prototype._arity = function(args, num, method, term) {
  ...
  if (foundArgs === false) {
    throw new errors.ReqlDriverError('`' + method + '` takes ' + num + ' argument' + ((num > 1) ? 's' : '') + ', ' + args.length + ' provided', term._query);
  }
};
...
function Func(r, func) {
  ...
  if (body === undefined)
    throw new errors.ReqlDriverError('Annonymous function returned `undefined`. Did you forget a `return`? In:\n' + func.toString(), this._query);
