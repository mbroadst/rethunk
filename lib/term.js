...
function Term(r, value, error) {
  ...
  term._query = (value === undefined) ? [] : value;
        ...
        if ((key === 'readMode') || (key === 'durability') || (key === 'db') ||
          ...
          if (key === 'db') {
            ...
            _options[key] = new Term(self._r).expr(options[key])._query;
                   ...
                   (key !== 'highWaterMark')) {
          reject(new errors.ReqlDriverError('Unrecognized option `' + key + '` in `run`. Available options are ' +
            'readMode <string>, durability <string>, noreply <bool>, timeFormat <string>, groupFormat: <string>, ' +
            'profile <bool>, binaryFormat <bool>, cursor <bool>, stream <bool>'));
          keepGoing = false;
        }
      }
    });
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'primaryKey') && (key !== 'durability') && (key !== 'shards') &&
        ...
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `tableCreate`', self._query,
          ...
          'replicas <number/object>, primaryReplicaTag <object>');
      }
    });
  ...
  if (helper.isPlainObject(options)) {
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'multi') && (key !== 'geo')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `indexCreate`', self._query,
          'Available option is multi <bool> and geo <bool>');
      }
    });
...
Term.prototype.changes = function(options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'squash') && (key !== 'includeStates') && (key !== 'includeInitial')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `changes`',
          self._query, 'Available options are: squash <bool>, includeStates <bool>, includeInitial <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability') && (key !== 'conflict')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `insert`', self._query,
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability') && (key !== 'nonAtomic')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `update`', self._query,
...
Term.prototype.replace = function(newValue, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability') && (key !== 'nonAtomic')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `replace`', self._query,
          'Available options are returnChanges <bool>, durability <string>, nonAtomic <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'returnChanges') && (key !== 'durability')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `delete`', self._query,
...
Term.prototype.getAll = function() {
  ...
  if ((_args.length > 1) && (helper.isPlainObject(_args[_args.length - 1])) &&
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'index') && (key !== 'leftBound') && (key !== 'rightBound')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `between`', self._query,
...
Term.prototype.eqJoin = function(rightKey, sequence, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'index') {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `eqJoin`', self._query,
...
Term.prototype.orderBy = function() {
  ...
  if ((_args.length > 0) && (helper.isPlainObject(_args[_args.length - 1])) &&
    ...
    if ((_args[_args.length - 1] instanceof Term) &&
        ((_args[_args.length - 1]._query[0] === termTypes.DESC) ||
          (_args[_args.length - 1]._query[0] === termTypes.ASC))) {
      args.push(new Term(this._r).expr(_args[_args.length - 1]));
    } else {
      args.push(new Term(this._r).expr(_args[_args.length - 1])._wrap());
    }
  ...
  if (helper.isPlainObject(options)) {
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((keepGoing === true) && (key !== 'index')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `distinct`', self._query,
          'Available option is index: <string>');
      }
    });
  ...
  if (_args.length > 0) {
    if (helper.isPlainObject(_args[_args.length - 1])) {
      helper.loopKeys(_args[_args.length - 1], function(obj, key) {
        if ((key !== 'index') && (key !== 'multi')) {
          throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `group`', self._query,
            'Available options are index: <string>, multi <boolean>');
        }
      });
      term._fillArgs(args);
...
Term.prototype.random = function() {
  ...
  if (_args.length > 0) {
    if (helper.isPlainObject(_args[_args.length - 1])) {
      helper.loopKeys(_args[_args.length - 1], function(obj, key) {
        if (key !== 'float') {
          throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `random`', self._query,
            'Available option is float: <boolean>');
        }
      });
      term._fillArgs(args);
      term._query.push(new Term(self._r).expr(translateOptions(_args[_args.length - 1]))._query);
...
Term.prototype.time = function() {
  ...
  if (foundArgs === false) {
    if ((_args.length !== 4) && (_args.length !== 7)) {
      throw new errors.ReqlDriverError('`r.time` called with ' + _args.length +
        ' argument' + ((_args.length > 1) ? 's' : ''), null, '`r.time` takes 4 or 7 arguments');
    }
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'defaultTimezone') {
        throw new errors.ReqlDriverError('Unrecognized option `' + key +
          '` in `ISO8601`. Available options are primaryKey <string>, durability <string>, datancenter <string>');
      }
    });
    term._query.push(new Term(this._r).expr(translateOptions(options))._query);
  }
...
Term.prototype.expr = function(expression, nestingLevel) {
  ...
  if (_nestingLevel < 0)
    throw new errors.ReqlDriverError('Nesting depth limit exceeded.\n' +
...
Term.prototype.http = function(url, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'timeout') && (key !== 'reattempts') && (key !== 'redirects') &&
          (key !== 'verify') && (key !== 'resultFormat') && (key !== 'method') &&
          (key !== 'auth') && (key !== 'params') && (key !== 'header') &&
          (key !== 'data') && (key !== 'page') && (key !== 'pageLimit') && (key !== '')) {
          throw new errors.ReqlDriverError('Unrecognized option `' + key +
            '` in `http`. Available options are reattemps <number>, redirects <number>, verify <boolean>, ' +
            'resultFormat: <string>, method: <string>, auth: <object>, params: <object>, header: <string>, ' +
            'data: <string>, page: <string/function>, pageLimit: <number>');
      }
    });
  ...
  if (helper.isPlainObject(options)) {
    ...
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'numVertices') && (key !== 'geoSystem') && (key !== 'unit') && (key !== 'fill')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `circle`', self._query,
          'Available options are numVertices <number>, geoSsystem <string>, unit <string> and fill <bool>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'geoSystem') && (key !== 'unit')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `distance`', self._query,
          'Available options are geoSystem <string>, unit <string>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
...
Term.prototype.getIntersecting = function(geometry, options) {
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if (key !== 'index') {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `distance`', self._query,
          'Available options are index <string>');
      }
    });
    term._query.push(new Term(this._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(options)) {
    helper.loopKeys(options, function(obj, key) {
      if ((key !== 'waitFor') && (key !== 'timeout')) {
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `wait`', self._query,
          'Available options are waitFor: <string>, timeout: <number>');
      }
    });
    term._query.push(new Term(self._r).expr(translateOptions(options))._query);
  }
  ...
  if (helper.isPlainObject(config)) {
    helper.loopKeys(config, function(obj, key) {
      if ((key !== 'shards') && (key !== 'replicas') &&
        ...
        throw new errors.ReqlDriverError('Unrecognized option `' + key + '` in `reconfigure`', self._query,
          'Available options are shards: <number>, replicas: <number>, primaryReplicaTag: <object>, ' +
          'dryRun <boolean>, emergencyRepair: <string>, nonvotingReplicaTags: <array<string>>');
      }
    });
...
Term.prototype.delay = function(delay) {
  return this.run().delay(delay);
};
...
Term.prototype._translateArgs = {
  ...
  nonvotingReplicaTags: 'nonvoting_replica_tags'
};
...
Term.prototype._arityRange = function(args, min, max, method, term) {
  ...
  if (args.length < min) {
    ...
    if (foundArgs === false) {
      throw new errors.ReqlDriverError('`' + method + '` takes at least ' + min +
        ' argument' + ((min > 1) ? 's' : '') + ', ' + args.length + ' provided', term._query);
    }
    ...
    if (foundArgs === false) {
      throw new errors.ReqlDriverError('`' + method + '` takes at most ' + max +
        ' argument' + ((max > 1) ? 's' : '') + ', ' + args.length + ' provided', term._query);
    }
  }
};
...
Term.prototype._arity = function(args, num, method, term) {
  ...
  if (foundArgs === false) {
    throw new errors.ReqlDriverError('`' + method + '` takes ' + num + ' argument' +
      ((num > 1) ? 's' : '') + ', ' + args.length + ' provided', term._query);
  }
};
...
function Func(r, func) {
  ...
  if (body === undefined)
    throw new errors.ReqlDriverError('Annonymous function returned `undefined`. ' +
      'Did you forget a `return`? In:\n' + func.toString(), this._query);
