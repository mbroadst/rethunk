...
Cursor.prototype._next = function(callback) {
  ...
  var p = new Promise(function(resolve, reject) {
    if (self._closed === true) {
      reject(new errors.ReqlDriverError('You cannot called `next` on a closed ' + this._type));
    }
    else if ((self._data.length === 0) && (self._canFetch === false)) {
      reject(new errors.ReqlDriverError('No more rows in the ' + self._type));
    }
    else {
...
Cursor.prototype._flush = function() {
  while ((this._pendingPromises.length > 0) && ((this._data.length > 0) || ((this._fetching === false) && (this._canFetch === false)))) {
    ...
    if (this._data.length > 0) {
      ...
      reject(new errors.ReqlDriverError('No more rows in the ' + this._type));
    }
  }
};
...
Cursor.prototype._makeEmitter = function() {
  this.next = function() {
    throw new errors.ReqlDriverError('You cannot called `next` once you have bound listeners on the ' + this._type);
  };
  this.each = function() {
    throw new errors.ReqlDriverError('You cannot called `each` once you have bound listeners on the ' + this._type);
  };
  this.toArray = function() {
    throw new errors.ReqlDriverError('You cannot called `toArray` once you have bound listeners on the ' + this._type);
  };
