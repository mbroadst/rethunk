...
Connection.prototype._handleRuntimeError = function(response, token) {
  ...
  if (typeof this.metadata[token].reject === 'function') {
    ...
    currentReject = this.metadata[token].reject;
    this.metadata[token].removeCallbacks();
    error = new errors.ReqlRuntimeError(helper.makeAtom(response), this.metadata[token].query, response);
...
Connection.prototype._handleSuccessAtom = function(response, token) {
  ...
  if ((Array.isArray(datum)) &&
    ...
    if (this.metadata[token].options.profile === true) {
      this.metadata[token].resolve({ profile: response.p, result: cursor });
    } else {
    ...
    if (this.metadata[token].options.profile === true) {
      this.metadata[token].resolve({ profile: response.p, result: stream });
    } else {
    ...
    if (this.metadata[token].options.profile === true) {
      result = { profile: response.p, result: cursor || datum };
    } else {
      result = datum;
    }
...
Connection.prototype._handleSuccessPartial = function(response, token) {
  ...
  if (!this.metadata[token].cursor) { // No cursor, let's create one
    ...
    if ((this.metadata[token].options.cursor === true) || ((this.metadata[token].options.cursor === undefined) && (this.r._options.cursor === true))) {
      // Return a cursor
      if (this.metadata[token].options.profile === true) {
        currentResolve({ profile: response.p, result: cursor });
      } else {
      ...
      if (this.metadata[token].options.profile === true) {
        currentResolve({ profile: response.p, result: stream });
      } else {
      ...
      cursor.toArray()
        .then(function(result) {
          if (options.profile === true) {
            currentResolve({ profile: response.p, result: result });
          } else {
