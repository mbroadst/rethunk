...
var dataHandler = function(self, resolve, reject) {
...
var connectHandler = function(self, resolve, reject) {
  return function() {
...
var closeHandler = function(self, resolve, reject) {
  return function() {
...
var endHandler = function(self, resolve, reject) {
  return function() {
    self.open = false;
    self.emit('end');
...
Connection.prototype.connect = function(options, resolve, reject) {
  var self = this;
  var connectionArgs = {
    host: self.host, port: self.port, family: net.isIPv6(self.host) ? 'IPv6' : 'IPv4'
  };
  ...
  self.connection.on('connect', connectHandler(self, resolve, reject));
  self.connection.on('close', closeHandler(self, resolve, reject));
  self.connection.on('end', endHandler(self, resolve, reject));
  self.connection.on('data', dataHandler(self, resolve, reject));
