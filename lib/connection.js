...
Connection.prototype._processResponse = function(response, token) {
  ...
  if (type === responseTypes.COMPILE_ERROR) {
    ...
    if (typeof self.metadata[token].reject === 'function') {
      self.metadata[token].reject(new errors.ReqlCompileError(helper.makeAtom(response), self.metadata[token].query, response));
    }
    ...
    if (typeof self.metadata[token].reject === 'function') {
      ...
      currentReject(new errors.ReqlClientError(helper.makeAtom(response), self.metadata[token].query, response));
    ...
    if (typeof self.metadata[token].reject === 'function') {
      ...
      currentReject = self.metadata[token].reject;
      self.metadata[token].removeCallbacks();
      error = new errors.ReqlRuntimeError(helper.makeAtom(response), self.metadata[token].query, response);
      ...
      currentReject = self.metadata[token].endReject;
      self.metadata[token].removeEndCallbacks();
      currentReject(new errors.ReqlRuntimeError(helper.makeAtom(response), self.metadata[token].query, response));
      delete self.metadata[token];
    }
  }
